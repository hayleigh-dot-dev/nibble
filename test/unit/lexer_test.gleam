...
pub fn single_digit_integer_test() {
  use test <- should("lex a sigle digit integer")

  let input = "1"
  let expected = [Token(Span(1, 1, 1, 2), "1", Int(1))]

  test(input, expected, [lexer.int(Int)])
}

pub fn single_negative_digit_integer_test() {
  use test <- should("lex a single digit negative integer")
...
pub fn multi_digit_integer_test() {
  use test <- should("lex a multi digit integer")
...
pub fn multi_digit_negative_integer_test() {
  use test <- should("lex a multi digit negative integer")
...
pub fn single_digit_float_test() {
  use test <- should("lex a single digit float")

  let input = "1.0"
  let expected = [Token(Span(1, 1, 1, 4), "1.0", Num(1.0))]

  test(input, expected, [lexer.float(Num)])
}

pub fn single_nagive_digit_float_test() {
  use test <- should("lex a single digit negative float")
...
pub fn multi_digit_float_test() {
  use test <- should("lex a multi digit float")
...
pub fn multi_digit_negative_float_test() {
  use test <- should("lex a multi digit negative float")
...
pub fn empty_string_test() {
  use test <- should("lex an empty string")

  let input = "''"
  let expected = [Token(Span(1, 1, 1, 3), "''", Str(""))]

  test(input, expected, [lexer.string("'", Str)])
}

pub fn single_char_string_test() {
  use test <- should("lex a single char string")
...
pub fn multi_char_string_test() {
  use test <- should("lex a multi char string")
...
pub fn escaped_string_test() {
  use test <- should("lex an escaped string")
...
pub fn multiline_string_test() {
  use test <- should("lex a multi-line string")
...
pub fn keyword_test() {
  use test <- should("lex a keyword")

  let input = "in"
  let expected = [Token(Span(1, 1, 1, 3), "in", Kwd("in"))]

  test(input, expected, [lexer.keyword("in", "\\s", Kwd("in"))])
}

pub fn keyword_breaker_test() {
  use test <- should("lex a keyword in an identifier")
...
pub fn single_char_variable_test() {
  use test <- should("lex a single char variable")

  let input = "a"
  let expected = [Token(Span(1, 1, 1, 2), "a", Var("a"))]

  test(input, expected, [lexer.variable(set.new(), Var)])
}

pub fn multi_char_variable_test() {
  use test <- should("lex a multi char variable")
...
pub fn multi_char_variable_with_numbers_and_underscores_test() {
  use test <- should("lex a multi char variable with numbers and underscores")
...
pub fn variable_containing_keyword_test() {
  use test <- should("lex a variable containing a keyword")
